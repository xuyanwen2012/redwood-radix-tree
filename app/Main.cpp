...
int main() {
  ...
  MakeUnlinkedOctreeNodes(u_bh_nodes.data(), u_oc_node_offsets.data(),
                          ...
                          u_brt_nodes.data(), num_brt_nodes, min_coord, range);
  ...
  for (int i = 0; i < num_oc_nodes; ++i) {
    ...
    std::cout << "\tchild_node_mask: "
              << std::bitset<8>(u_bh_nodes[i].child_node_mask) << "\n";
    std::cout << "\tchild_leaf_mask: "
              << std::bitset<8>(u_bh_nodes[i].child_leaf_mask) << "\n";

    std::cout << "\tchild : [" << u_bh_nodes[i].children[0];
    for (int j = 1; j < 8; ++j) {
      std::cout << ", " << u_bh_nodes[i].children[j];
    }
    std::cout << "]\n";

    std::cout << "\tcell_size: " << u_bh_nodes[i].cell_size << "\n";
    std::cout << "\tcornor: (" << u_bh_nodes[i].cornor.transpose() << ")\n";
    std::cout << "\n";
  }
  ...
  //   auto x_range =
  //       std::minmax_element(u_inputs.begin(), u_inputs.end(),
  //       CompareAxis<0>);
  //   auto y_range =
  //       std::minmax_element(u_inputs.begin(), u_inputs.end(),
  //       CompareAxis<1>);
  ...
  //   MakeNodes(u_bh_nodes.data(), u_oc_node_offsets.data(),
  //   u_edge_count.data(),
  ...
  // // std::cout << "Actual number of threads: " << actual_threads <<
  // std::endl;
